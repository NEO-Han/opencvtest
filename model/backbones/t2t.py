...
class T2T(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, tokens_type='transformer', in_chans=3, embed_dim=768, token_dim=64):
        ...
        # self.num_patches = (img_size[0] // (kernel_size[0][1] * kernel_size[1][1] * kernel_size[2][1])) * (img_size[1] // (kernel_size[0][1] * kernel_size[1][1] * kernel_size[2][1]))  # there are 3 sfot split, stride are 4,2,2 seperately
        
        
        self.num_x=img_size // (kernel_size[0][1] * kernel_size[1][1] * kernel_size[2][1])
        self.num_y=img_size // (kernel_size[0][1] * kernel_size[1][1] * kernel_size[2][1])
    ...
    def forward(self, x):
        ...
        x = self.attention1(x)
        B, new_HW, C = x.shape
        x = x.transpose(1,2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))#shape '[8, 64, 45, 45]' is invalid for input of size 1048576
